...
# --- Состояние атаки (для индикаторов) ---
...
func _ready() -> void:
    ...
    # ВЫЗОВ ФУНКЦИИ (которая вызывала ошибку)
    _update_visuals()
    _apply_level_scale()
...
func _process(delta: float) -> void:
    ...
    _process_damage_tracking(delta)
    
    if alignment != Alignment.NEUTRAL:
        _handle_tiers(delta)
        _process_auto_repair(delta)

# --- ЛОГИКА ЗАЩИТЫ И УРОНА ---

func _process_damage_tracking(delta: float) -> void:
    if is_instance_valid(health_component):
        if health_component.current_health < _last_health:
            var damage_taken = _last_health - health_component.current_health
            if damage_taken >= SERIOUS_DAMAGE_THRESHOLD:
...
func _apply_damage(amount: float) -> void:
    ...
    if amount >= SERIOUS_DAMAGE_THRESHOLD:
        is_under_attack = true
        _attack_timer = 4.0
...
func reinforce() -> void:
    if is_instance_valid(health_component):
        health_component.current_health = min(health_component.current_health + 1000, health_component.max_health)
...
func _handle_capture(delta: float) -> void:
    ...
    if (player_inside or invaders > 0) and alignment != Alignment.PLAYER:
        var p_bonus = (p_ref.mass / 20.0) if player_inside else 0.0
...
func _fire_at_enemy() -> void:
    ...
    var priority = targets.filter(func(t): return is_instance_valid(t) and t is Enemy and t.current_archetype == Enemy.Archetype.BREAKER)
    var final_targets = priority if not priority.is_empty() else targets
    ...
    if closest:
        ...
        b.faction = "player" if alignment == Alignment.PLAYER else "rival"
        
        var dmg = 10.0 + (current_level * 5.0)
        if specialty == Specialty.MILITARY: dmg *= 2.0
...
func upgrade(amount: float) -> void:
    ...
    var threshold = 150.0 * current_level
...
func apply_specialty(type: int) -> void:
    specialty = type as Specialty
    unit_cap = 15 if specialty == Specialty.MILITARY else 10
    if specialty == Specialty.INDUSTRY:
