...
func _on_health_depleted() -> void:
    ...
    if current_level > 1:
        ...
        # ИСКЛЮЧЕНИЕ НЕЙТРАЛЬНОГО СОСТОЯНИЯ:
        # Если лагерь игрока уничтожен - он становится вражеским
        # Если лагерь врага уничтожен - он становится союзным
        if alignment == Alignment.PLAYER:
            _flip_to(Alignment.RIVAL)
        elif alignment == Alignment.RIVAL:
            _flip_to(Alignment.PLAYER)
        else:
            # Если вдруг уничтожили нейтральный (не захваченный)
...
func _spawn_unit() -> void:
    ...
    u.alignment = int(alignment); u.parent_camp = self
...
func _fire_at_enemy() -> void:
    ...
    var priority_targets = targets.filter(func(t): 
    ...
    if closest:
        ...
        var base_dmg = 10.0 + (current_level * 5.0)
        if specialty == Specialty.MILITARY: base_dmg *= 2.0
...
func _flip_to(new_align: int) -> void:
    ...
    _update_visuals()
    if is_instance_valid(health_component):
        health_component.max_health = BASE_HEALTH; health_component.current_health = BASE_HEALTH
