...
var guard_radius: float = 350.0  # banner radius + 50px soft boundary
...
func _physics_process(delta: float) -> void:
	...
	if is_instance_valid(guard_target):
		var g_dist = global_position.distance_to(guard_target.global_position)
		var has_enemy = is_instance_valid(target)
		var enemy_dist = INF
		if has_enemy:
			enemy_dist = global_position.distance_to(target.global_position)
		
		# SOFT BOUNDARY: только если юнит слишком далеко от знамени, возвращаем
		if g_dist > guard_radius:
			# Force return to banner zone
			var g_dir = (guard_target.global_position - global_position).normalized()
			velocity = (g_dir * speed + separation).normalized() * speed
			if not is_attacking:
				if animated_sprite.animation != "Run": animated_sprite.play("Run")
				if g_dir.x != 0: animated_sprite.flip_h = (g_dir.x < 0)
			move_and_slide()
			return
		
		# DEADZONE: на позиции, нет врагов — Idle
		...
		# PRIORITY: есть враг — атаковать/бежать к нему
		if has_enemy:
			if enemy_dist < 60.0:
				velocity = separation
		...
		# MARCH: идти к guard_target если не близко
		if g_dist >= 40.0:
			var g_dir = (guard_target.global_position - global_position).normalized()
			velocity = (g_dir * speed + separation).normalized() * speed
			if not is_attacking:
...
func _get_separation_velocity() -> Vector2:
	...
	var friends = get_tree().get_nodes_in_group("units")
	for friend in friends:
		if friend == self or not is_instance_valid(friend):
			continue
		if ("alignment" in friend and friend.alignment != alignment):
			continue
		var d = global_position.distance_to(friend.global_position)
		if d < 30.0 and d > 0.01:
			sep -= (friend.global_position - global_position).normalized() * (30.0 - d) * 0.8
	return sep
