...
@export var alignment: int = 1 # 1=Player (Синий), 2=Rival (Красный)
...
func _physics_process(delta: float) -> void:
    ...
    if is_instance_valid(target):
        # ПРИОРИТЕТ ДЛЯ СИНИХ: защищать от Брекеров
        if alignment == 1 and not (target is Enemy and target.current_archetype == Enemy.Archetype.BREAKER):
...
func _toggle_hitbox() -> void:
    if is_instance_valid(hitbox):
        ...
        if shape:
            shape.disabled = true
            get_tree().create_timer(0.1).timeout.connect(func(): if is_instance_valid(shape): shape.disabled = false)

func _find_target() -> void:
    if is_instance_valid(target): return
    
    var potential = []
    if alignment == 1: # СИНИЕ ЮНИТЫ
        # Атакуют: Мобы (enemy), Красные юниты, Красные лагеря
        ...
        # Атакуют: Игрок, Синие юниты, Синие лагеря (ИГНОРИРУЮТ группу enemy)
...
func _get_nearby_enemy_breaker() -> Node2D:
    # Только синие юниты ищут Брекеров среди мобов
...
func flip_alignment(new_align: int) -> void:
    alignment = new_align; _setup_factions(); _update_visuals(); target = null
