...
func _physics_process(delta: float) -> void:
    ...
    # Обработка дебаффа от Disruptor (фиолетовые враги)
    var current_debuff_mod = 1.0
    if _disruptor_debuff_timer > 0:
        _disruptor_debuff_timer -= delta
        current_debuff_mod = 0.5
        if _disruptor_debuff_timer <= 0: modulate = Color.WHITE
    
    var input_vector = Input.get_vector("move_left", "move_right", "move_up", "move_down")
    
    # Движение с учетом всех модификаторов
    _move_player(delta, input_vector, current_debuff_mod)
...
# --- ДВИЖЕНИЕ ---
func _move_player(delta: float, input_vector: Vector2, debuff_mod: float) -> void:
    var mass_penalty = base_mass / mass
    # Итоговая скорость включает: Базу * Бафф Лагеря * Мод Зоны * Дебафф Врага * Штраф Массы
    var current_speed = max_speed * (1.0 + camp_buffs.speed) * applied_zone_speed_modifier * debuff_mod * mass_penalty
